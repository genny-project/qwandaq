...
public class QuestionUtils implements Serializable {
	...
	public static void setCachedQuestionsRecursively(Ask ask, String token) {
		...
		if (ask.getAttributeCode().equals("QQQ_QUESTION_GROUP")) {
			...
			String jsonQuestionStr = (String) CacheUtils.readCache(gToken.getRealm(), questionCode);
			JsonObject jsonQuestion = toJson(jsonQuestionStr);

			if ("ok".equalsIgnoreCase(jsonQuestion.getString("status"))) {
				Question cachedQuestion = jsonb.fromJson(jsonQuestion.getString("value"), Question.class);
	...
	static public Question getQuestion(String questionCode, GennyToken userToken) {
		...
		while (retry >= 0) { // Sometimes q is read properly from cache
			String qJsonStr = (String) CacheUtils.readCache(userToken.getRealm(), questionCode);
	...
	public static List<BaseEntity> getChildren(String beCode, Integer level, String token) {
		...
		if (parent != null) {
			for (EntityEntity ee : parent.getLinks()) {
				...
				BaseEntity child = CacheUtils.getObject(gennyToken.getRealm(), childCode, BaseEntity.class);
				result.add(child);
			}
		}
